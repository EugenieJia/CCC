...
insta_sim <- function(m, rhog, dist, mean_g, covar_g){
  m <- m
  dist <- dist
  mean_g <- mean_g
  covar_g <- covar_g
...
# collect data
n <- 10
dat <- infer(m,n,rhog,dist,mean_g,covar_g)
n10 <- insta_table(dat)
...
lin_run <- function(dist, sel){
  ...
  if(1 %in% sel){
    ...
    covar_g <- mat1
    rhog <- 0.95
    case_1 <<- insta_sim(m,rhog,dist,mean_g,covar_g)
  }
  if(2 %in% sel){
    mean_g <- mu1
    covar_g <- mat1
    rhog <- 0.95
    case_2 <<- insta_sim(m,rhog,dist,mean_g,covar_g)
  }
  if(3 %in% sel){
    mean_g <- mu1
    covar_g <- mat2
    rhog <- 0.95
    case_3 <<- insta_sim(m,rhog,dist,mean_g,covar_g)
  }
  if(4 %in% sel){
    mean_g <- mu1
    covar_g <- mat3
    rhog <- 0.8
    case_4 <<- insta_sim(m,rhog,dist,mean_g,covar_g)
  }
  if(5 %in% sel){
    mean_g <- mu2
    covar_g <- mat4
